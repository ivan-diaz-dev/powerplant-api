...
def calculate_production_plan(
    ...
    try:
        ...
        logger.warning("Invalid production plan request", exc_info=(exc.__class__, exc, exc.__traceback__))
        raise fastapi.HTTPException(status_code=fastapi.status.HTTP_400_BAD_REQUEST, detail=str(exc)) from None
    except Exception:  # TODO: search for a more specific exception to catch here if is posible.
